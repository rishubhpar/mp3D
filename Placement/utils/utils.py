...
def theta2alpha_3d(theta, x, z, P2):
    ...
    """If two images are coplanar, i.e. they were taken such that the right camera is only offset horizontally compared to the left camera 
    (not being moved towards the object or rotated), then each pixel’s epipolar line is horizontal and at the same vertical position as that 
    pixel. """
    offset = P2[0, 3] / P2[0, 0] #rectifed exrinsics
...
def compound_annotation_detr(labels,max_length,bbox2d,bbox_3d,obj_types):
...
def compound_annotation(labels, max_length, bbox2d, bbox_3d, obj_types):
    ...
    for i in range(len(labels)):
        label = labels[i]
        for j in range(len(label)):
            # print ("CHECK HERER ############:",obj_types.index(label[j]))
            annotations[i, j] = np.concatenate([
                bbox2d[i][j], [obj_types.index(label[j])], bbox_3d[i][j]
            ])
    return annotations
